...
from transpose.utils.exceptions import ChartConfigError
...
class Chart:
    ...
    def __init__(self, api_key: str) -> None:
        ...
        if api_key is None or not isinstance(api_key, str) or len(api_key) <= 0: 
            raise ChartConfigError('Transpose API key is required')
        self.api_key = api_key
    ...
    def ohlc(self, chain: str, token_address: str, from_timestamp: str, to_timestamp: str, timeframe: str,
        ...
        if chain not in ['ethereum', 'goerli', 'polygon']:
            raise ChartConfigError('Invalid chain')
        
        # validate token address
        token_address = to_checksum_address(token_address)
        if token_address is None: raise ChartConfigError('Invalid token address')

        # validate timestamps
        try: from_timestamp = to_iso(from_timestamp)
        except Exception as e: raise ChartConfigError('Invalid from timestamp') from e
        try: to_timestamp = to_iso(to_timestamp)
        except Exception as e: raise ChartConfigError('Invalid to timestamp') from e

        # validate timeframe
        if timeframe not in ['day', 'hour', 'minute']:
            raise ChartConfigError('Invalid timeframe')
